...
-- new _r
    local i2h_r = nn.Linear(params.rnn_size, 4 * params.rnn_size)(x_r)
    local h2h_r = nn.Linear(params.rnn_size, 4 * params.rnn_size)(next_h)
...
local function training(model, round)
    ...
    while epoch < params.max_max_epoch do
        ...
        if step % torch.round(epoch_size / 10) == 10 then
            ...
            if new_train_perp > last_train_perp or (last_train_perp - new_train_perp) < params.lr then 
        ...
        if step % epoch_size == 0 then
            ...
            if new_valid_perp > last_valid_perp + params.tolerance then  -- or params.lr < params.minlr  then --or step / epoch_size > 1.0 then -- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! for fast check, later delete
                params.lr = params.lr / params.decay
            end
...
local function updatebest_c(model, info, round, state)
    ...
    --for j = 1, 1 do
        ...
        for i = 1, params.seq_length do
            ...
            --local lo = model.rnns[i]:findModules('nn.LogSoftMax')[1].output
            --lo = torch.DoubleTensor(lo:size()):copy(lo)
...
local function gao()
    ...
    if arg[1] == nil then print("useage: th main_c.lua ../../dataset/big-data/xx/ 2>&1 | tee log.txt") return end
    data_path = arg[1]
    ...
    print("vacab = "..vocab)
    base  = math.ceil(math.sqrt(vocab)) print("base = "..base)
...
local function generate(model, rmapxy, length, base, batch_size, pre_sentence) -- (len x batch_size)
    if model ~= nil and model.start_s ~= nil then for d = 1, 2 * params.layers do model.start_s[d]:zero() end end
    g_replace_table(model.s[0], model.start_s)
    g_disable_dropout(model.rnns)

--  init first words, left part
    local last_x = torch.zeros(batch_size)
    local null = last_x:clone():cuda()
    local answer = {}

    for step = 1, pre_sentence:size(1), 2 do
        table.insert(answer, pre_sentence[step])
        table.insert(answer, pre_sentence[step + 1])
        local x = pre_sentence[step]:cuda()
        local x_r = pre_sentence[step + 1]:cuda()
        local y = null local y_r = null
        _, _, model.s[1] = unpack(model.rnns[1]:forward({x, y, model.s[0], x_r, y_r}))
        g_replace_table(model.s[0], model.s[1])

        local lsm = model.rnns[1]:findModules('nn.LogSoftMax')
        local lox = lsm[2].output
        lox = torch.DoubleTensor(lox:size()):copy(lox)
        assert(lox:size(1) == batch_size)
        local maxs, indices = torch.max(lox, 2)
        last_x = indices:squeeze():clone()
    end

    table.insert(answer, last_x) -- for first x
    ...
    for step = 1, length do
        ...
        local y   = null
        local y_r = null
        model.rnns[1]:forward({x, y, model.s[0], x_r, y_r})
        local lsm = model.rnns[1]:findModules('nn.LogSoftMax')
        local loy = lsm[1].output
        loy = torch.DoubleTensor(loy:size()):copy(loy)
        assert(loy:size(1) == batch_size)
        local maxs, indices = torch.max(loy, 2)
        table.insert(answer, indices:squeeze():clone()) -- for y

        local x_r = indices:squeeze():cuda()
        _, _, model.s[1] = unpack(model.rnns[1]:forward({x, y, model.s[0], x_r, y_r}))
        local lsm = model.rnns[1]:findModules('nn.LogSoftMax')
        local lox = lsm[2].output
        lox = torch.DoubleTensor(lox:size()):copy(lox)
    ...
    local sentences = {}
    for i = 1, #answer, 2 do
        if i + 1 > #answer then break end
        local number = answer[i] * jinzhi + answer[i + 1]
        local words = {}
        assert(number:size(1) == batch_size)
        table.insert(sentences, rmapxy[number[1] ])
    end
...
local function gaogenerate()
    g_init_gpu({}) -- gpu is nil, then we use the first gpu by default
    if arg[1] == nil then print("useage: th main_c.lua ../../dataset/big-data/xx/ 2>&1 | tee log.txt") return end
    data_path = arg[1]
    vocab = 0
    local word_list = {}
    file = io.open(data_path.."/idx2word.txt")
    while true do
        local str = file:read()
        if str == nil then break end
    ...
    local check = torch.load("./implus_round17_epoch6.00_93.67.test94.603.model.t7")
    model = check.model
    mapx  = check.mapx
    mapy  = check.mapy
    check_conflict = ptb.check_conflict
    ...
    local pre_string ="we are going" --"these are" --"just baby steps toward real interactive video which i believe will be the biggest thing yet to affect television he says </S>"
    local string_arr = stringx.split(pre_string)
    local pre_num    = #string_arr

    local pre_array  = {"the", "we", "we are", "we can", "it is", "today", "we are going"}
    print("original prefix sentences:")
    print(pre_array)

    for s = 1, #pre_array do
        local pre_string = pre_array[s]
        local string_arr = stringx.split(pre_string)
        local pre_num    = #string_arr
        local pre_sentence = torch.zeros(pre_num * 2, params.batch_size)
        rmapxy = {}
        for w = 1, vocab do
            rmapxy[mapx[w] * jinzhi + mapy[w]] = word_list[w]
            for i = 1, #string_arr do
                if word_list[w] == string_arr[i] then
                    pre_sentence[i * 2 - 1][1] = mapx[w]
                    pre_sentence[i * 2][1] = mapy[w]
                end
            end
        end
        --local function generate(model, rmapxy, length, base, batch_size)
        generate(model, rmapxy, 10, params.vocab_code, params.batch_size, pre_sentence)
    end
end
