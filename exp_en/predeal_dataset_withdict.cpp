...
int main(int argc, char* argv[]){
    if(argc < 6) {
        cout<<"./predeal_dataset ../../dataset/big-data/es/train.txt ../../dataset/big-data/es/valid.txt ../../dataset/big-data/es/test.txt ../../dataset/big-data/es/ data/en-fr.src.dict"<<endl;
        return 1;
    }
    ...
    prt("fill vocabulary! from predefined vocabulary file");
    int idx = 0;
    ...
    */
    fstream f_vocab(argv[5], ios::in);
    string token, tokenid; int tokenidint;
    while(f_vocab >> token >> tokenid){
        stringstream ss(tokenid);
        ss >> tokenidint;
        word2idx[token] = tokenidint;
        assert(tokenidint == ++idx);
    }
